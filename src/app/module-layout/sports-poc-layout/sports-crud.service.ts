  ...
  async createSport(createSportObj: any) {
    ...
    try {
      if (!createSportObj.name) {
        ...
        let sportAlreadyExist = await this.adminref.collection('/sports').doc(uniqueSportName).get();
        if (sportAlreadyExist.exists) {
          return { 'status': false, 'message': 'Sport is already exist.' }
        } else {
          //Create Sport

          let createSport: any = await this.adminref.collection('/sports').doc(uniqueSportName).set(sportObj, { merge: true });
          console.log(createSport);
          return { 'status': true, 'message': 'Sport created successfully.' }
          //   this.adminref.collection('/sports').doc(uniqueSportName).set(sportObj).then(sportResponse => {
          //     console.log(sportResponse);

          //     return { 'status': false, 'message': 'Sport created successfully' }
          //     // response.status(200).send({ 'status': true, 'message': 'Sports created successfully' })
          // }).catch(error => {
    ...
    try {
      if (!updateSportObj.sport_id) {
        ...
        if (updateSport.exists) {
          ...
          let obj = {
            name: sportName, sport: sportName,created_datetime:updateSportObj.created_datetime
          }
