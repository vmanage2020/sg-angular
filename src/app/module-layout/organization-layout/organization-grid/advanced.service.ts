...
import { SortDirection } from '../../../dataTable/advanced-sortable.directive';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.name.toLowerCase().includes(term)
        || tables.abbreviation.toLowerCase().includes(term)
        || tables.state.toLowerCase().includes(term)
        || pipe.transform(tables.mobile).includes(term)
