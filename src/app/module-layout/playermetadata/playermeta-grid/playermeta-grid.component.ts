...
@Component({
  selector: 'app-playermeta-grid',
  templateUrl: './playermeta-grid.component.html',
  styleUrls: ['./playermeta-grid.component.scss']
})
export class PlayermetaGridComponent implements OnInit {
  ...
  sportsInfo: any;
  noDataDefault = true
  initalValue: any = "All Player Custom Field";
  ...
  public PlayerMetadataService: PlayerMetadataService) { 
    ...
    if (this.injectedData) {
      if (this.injectedData.data) {
        if(this.injectedData.data.userAction === "create" && this.injectedData.data.userAction){
          ...
          this.playermetaInfo = this.injectedData.data.userInfo;
          ...
          if ((this.injectedData.data.pageSize && this.injectedData.data.selectedSport) || (this.injectedData.data.pageSize && this.injectedData.data.selectedSportName === this.initalValue)) {
            ...
            if (this.nextEnabled) {
              this.getPlayerMetaList(this.uid, this.orgId, this.sportId, this.roleId, this.injectedData.data.pageNo, this.injectedData.data.pageSize,
            ...
            this.sportId = this.injectedData.data.sport_id
            this.sportName = this.injectedData.data.sport_name;
            this.getPlayerMetaList(this.uid, this.orgId, this.injectedData.data.sport_id, this.roleId, this.page,
        ...
        this.getPlayerMetaList(this.uid, this.orgId, this.sportId, this.roleId, this.page, this.pageSize, this.nextEnabled, this.prevEnabled, null, this.firstInResponse, this.lastInResponse)
      }
    } else {
      this.getPlayerMetaList(this.uid, this.orgId, this.sportId, this.roleId, this.page, this.pageSize, this.nextEnabled, this.prevEnabled, null, this.firstInResponse, this.lastInResponse)
    }
  ...
  addPlayermeta() {
    ...
    this.change.emit({
      action: "createplayermetadata", data: this.getUserListInfo
    })
  }
  ...
  async getPlayerMetaList(uid, orgId, sportId, roleId, pageNo, itemPerPage, isNextReq, isPrevReq, prevStartAt, prevEndAt, nextStartAt) {

    console.log("uid="+uid+" orgId="+orgId+" sportId="+sportId+" roleId="+roleId+" pageNo="+pageNo+" itemPerPage="+itemPerPage+" isNextReq="+isNextReq+" isPrevReq="+isPrevReq+" prevStartAt="+prevStartAt+" prevEndAt="+prevEndAt+" nextStartAt="+nextStartAt);
    ...
    try {
      ...
      let getplayermeta: any = await this.PlayerMetadataService.getPlayermetadata(getUserList);    
      if (getplayermeta.status) {
        if (getplayermeta.data) {
          if (typeof (getplayermeta.data) !== "string") {
            ...
            console.log("DATASA"+getplayermeta.data);

            this.playermetaInfo = getplayermeta.data;
      ...
      console.log(error);
      this.noData();
      this.notification.isNotification(true, "Player Custom Field", error.message, "times-circle");
  ...
  loaddataAfterCreate(data: any) {
    ...
    this.playermetaInfo = data.userInfo;
  ...
  editplayerfield(data: any) {
    this.loadData(data);
    data.viewBy = "edit";
    this.change.emit({ action: "editplayerfield", data: data })
  }

  viewplayer(data: any) {
    this.loadData(data);  
    this.change.emit({ action: "viewplayerfield", data: data })
  }
  
  
  async deleteplayerfield(data: any) {
    try {
      this.notification.isConfirmation('', '', 'Player Custom Field', ' Are you sure to delete ' + data.field_name + ' ?', 'question-circle', 'Yes', 'No', 'custom-ngi-confirmation-wrapper').then(async (dataIndex) => {
        if (dataIndex[0]) {
          console.log("yes");
          let isDeleted: any = await this.PlayerMetadataService.deleteplayerfield(data);
          if (isDeleted) {
            if (isDeleted.status) {
              ...
              this.getPlayerMetaList(this.uid, this.orgId, this.sportId, this.roleId, this.page, this.pageSize, this.nextEnabled, this.prevEnabled, null, this.firstInResponse, this.lastInResponse);
              this.notification.isNotification(true, "Player Custom Field", isDeleted.message, "check-square");
            } else {
              this.notification.isNotification(true, "Player Custom Field", isDeleted.message, "times-circle");
              this.dropDownService.getNotificationForError();
            }
          } else {
            this.notification.isNotification(true, "Player Custom Field", "Unable to delete.Please try again later.", "times-circle");
            this.dropDownService.getNotificationForError();
          }
        } else {
          /* Do nothing */
    ...
    try {
      ...
      if (allSportResponse.status) {
        if (allSportResponse.data) {
          allSportResponse.data.splice(0, 0, { name: 'All Player Custom Field', sport_id: null });
          allSportResponse.data.forEach(element => {
            if (element.sport_id !== null) {
              element.value = "Player Custom Field By Sports"
            }
            else {
              element.value = null
            }
    ...
    try {
      if (getSportsByOrganizationResponse.status) {
        if (getSportsByOrganizationResponse.data) {
          getSportsByOrganizationResponse.data.splice(0, 0, { name: 'All Player Custom Field', sport_id: null });
  ...
  async prevPage() {
    try {
      ...
      this.reInitialiseGrid();
      await this.getPlayerMetaList(this.uid, this.orgId, this.sportId, this.roleId, this.page, this.pageSize, this.nextEnabled, this.prevEnabled, this.get_prev_startAt(this.page), this.firstInResponse, this.lastInResponse);
    } catch (error) {
  ...
  async nextPage() {
    try {
      ...
      this.page += 1;
      this.reInitialiseGrid();
      await this.getPlayerMetaList(this.uid, this.orgId, this.sportId, this.roleId, this.page, this.pageSize, this.nextEnabled, this.prevEnabled, null, this.firstInResponse, this.lastInResponse);
    } catch (err) {
