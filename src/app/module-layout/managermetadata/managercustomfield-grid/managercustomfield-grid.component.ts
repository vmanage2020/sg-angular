...
@Component({
  selector: 'app-managercustomfield-grid',
  templateUrl: './managercustomfield-grid.component.html',
  styleUrls: ['./managercustomfield-grid.component.scss']
})
export class ManagercustomfieldGridComponent implements OnInit {
  @Output() change = new EventEmitter();

  getManagerfieldListInfo: any = {};
  loaderInfo = new BehaviorSubject<Logoinfo>({ progressBarLoading: 0, statusOfProgress: Constant.gridLoadingMsg });
  loading = false;
  displayLoader: any = true;
  sportsInfo: any;
  noDataDefault = true
  initalValue: any = "All Manager Custom Field";
    ...
    if (this.injectedData) {
      if (this.injectedData.data) {
        if(this.injectedData.data.userAction === "create" && this.injectedData.data.userAction){
          ...
          this.managerfieldInfo = this.injectedData.data.userInfo;
          this.getManagerfieldListInfo = this.injectedData.data.requestData;
          ...
          if ((this.injectedData.data.pageSize && this.injectedData.data.selectedSport) || (this.injectedData.data.pageSize && this.injectedData.data.selectedSportName === this.initalValue)) {
            ...
            if (this.nextEnabled) {
              this.getManagercustomfield(this.uid, this.orgId, this.sportId, this.roleId, this.injectedData.data.pageNo, this.injectedData.data.pageSize,
            ...
            this.sportId = this.injectedData.data.sport_id
            this.sportName = this.injectedData.data.sport_name;
            this.getManagercustomfield(this.uid, this.orgId, this.injectedData.data.sport_id, this.roleId, this.page,
        ...
        this.getManagercustomfield(this.uid, this.orgId, this.sportId, this.roleId, this.page, this.pageSize, this.nextEnabled, this.prevEnabled, null, this.firstInResponse, this.lastInResponse)
      }
    } else {
      this.getManagercustomfield(this.uid, this.orgId, this.sportId, this.roleId, this.page, this.pageSize, this.nextEnabled, this.prevEnabled, null, this.firstInResponse, this.lastInResponse)
    } 
  ...
  loaddataAfterCreate(data: any) {
    ...
    this.managerfieldInfo = data.userInfo;
    this.getManagerfieldListInfo = data.requestData;
  ...
  loadData(data: any) {
    ...
    data.requestPayloadGrid = this.getManagerfieldListInfo;
  }
    ...
    try {
      ...
      if (allSportResponse.status) {
        if (allSportResponse.data) {
          allSportResponse.data.splice(0, 0, { name: 'All Manager Custom field', sport_id: null });
          allSportResponse.data.forEach(element => {
            if (element.sport_id !== null) {
              element.value = "All Manager Custom field"
            }
            else {
              element.value = null
            }
    ...
    try {
      if (getSportsByOrganizationResponse.status) {
        if (getSportsByOrganizationResponse.data) {
          getSportsByOrganizationResponse.data.splice(0, 0, { name: 'All Manager Custom field', sport_id: null });
  ...
  async prevPage() {
    try {
      ...
      this.reInitialiseGrid();
      await this.getManagercustomfield(this.uid, this.orgId, this.sportId, this.roleId, this.page, this.pageSize, this.nextEnabled, this.prevEnabled, this.get_prev_startAt(this.page), this.firstInResponse, this.lastInResponse);
    } catch (error) {
  ...
  async nextPage() {
    try {
      ...
      this.page += 1;
      this.reInitialiseGrid();
      await this.getManagercustomfield(this.uid, this.orgId, this.sportId, this.roleId, this.page, this.pageSize, this.nextEnabled, this.prevEnabled, null, this.firstInResponse, this.lastInResponse);
    } catch (err) {
      this.disable_next = false;
    }
  }
  
  async getManagercustomfield(uid, orgId, sportId, roleId, pageNo, itemPerPage, isNextReq, isPrevReq, prevStartAt, prevEndAt, nextStartAt) {
    try {
      this.loading = true;
      ...
      this.getManagerfieldListInfo = getUserList;
      // get service for fetch levels      
      let getplayermeta: any = await this.ManagerCustomFieldService.getManagerCustomFielddata(getUserList);    
      if (getplayermeta.status) {
        if (getplayermeta.data) {
          if (typeof (getplayermeta.data) !== "string") {
            ...
            this.managerfieldInfo = getplayermeta.data;
      ...
      console.log(error);
      this.noData();
      this.notification.isNotification(true, "Manager Custom Field", error.message, "times-circle");
      this.dropDownService.getNotificationForError();
    }
  }
  
  addManagerField() {
    this.getManagerfieldListInfo.isNextReq = false;
    this.getManagerfieldListInfo.isPrevReq = false;
    this.getManagerfieldListInfo.page_no = 1;
    this.getManagerfieldListInfo.item_per_page = 10;
    this.getManagerfieldListInfo.searchVal = '';
    this.getManagerfieldListInfo.sport_id = this.sportId;
    this.getManagerfieldListInfo.sport_name = this.sportName;
    this.change.emit({
      action: "createmanagerfield", data: this.getManagerfieldListInfo
    })
  }
  
  editmanagerfield(data: any) {
    this.loadData(data);
    data.viewBy = "edit";
    this.change.emit({ action: "editmanagerfield", data: data })
  }

  viewmanagerfield(data: any) {
    this.loadData(data);  
    this.change.emit({ action: "viewmanagerfield", data: data })
  }
  

  async deletemanagerfield(data: any) {
    try {
      this.notification.isConfirmation('', '', 'Manager Custom Field', ' Are you sure to delete ' + data.field_name + ' ?', 'question-circle', 'Yes', 'No', 'custom-ngi-confirmation-wrapper').then(async (dataIndex) => {
        if (dataIndex[0]) {
          console.log("yes");
          let isDeleted: any = await this.ManagerCustomFieldService.deleteManagerfield(data);
          if (isDeleted) {
            if (isDeleted.status) {
              this.prevEnabled = false;
              this.nextEnabled = false;
              this.reInitialiseGrid();
              this.getManagercustomfield(this.uid, this.orgId, this.sportId, this.roleId, this.page, this.pageSize, this.nextEnabled, this.prevEnabled, null, this.firstInResponse, this.lastInResponse);
              this.notification.isNotification(true, "Manager Custom Field", isDeleted.message, "check-square");
            } else {
              this.notification.isNotification(true, "Manager Custom Field", isDeleted.message, "times-circle");
              this.dropDownService.getNotificationForError();
            }
          } else {
            this.notification.isNotification(true, "Manager Custom Field", "Unable to delete.Please try again later.", "times-circle");
            this.dropDownService.getNotificationForError();
          }
        } else {
          /* Do nothing */
